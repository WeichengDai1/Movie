package edu.uci.ics.weiched.service.idm.security;
...
public class Session {
     ...
     */
    public Session(String email, Token sessionID, Timestamp timeCreated, Timestamp lastUsed, Timestamp exprTime) {
        this.email = email;
     ...
     */
    public void update() {
        if (!isDataTimedOut()&& !isDataExpired()) {
...
//        }
    ...
    public boolean NoNeedToReLogin(){
        Timestamp currentTime = new Timestamp(System.currentTimeMillis());
        Timestamp timeoutTime = new Timestamp(exprTime.getTime() - SESSION_TIMEOUT);
        if (currentTime.after(timeoutTime)) {
            return false;
        }
        return true;
